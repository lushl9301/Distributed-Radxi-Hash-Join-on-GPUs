...
LocalPartitioning::LocalPartitioning(uint64_t innerPartitionSize,
  ...
  JOIN_ASSERT(
	  hpcjoin::core::Configuration::CACHELINE_SIZE_BYTES == LOCAL_PARTITIONING_CACHELINE_SIZE,
	  "Local Partitioning",
...
void LocalPartitioning::execute() {
  ...
  uint64_t innerOutputSize = (innerPartitionSize +
	  hpcjoin::core::Configuration::LOCAL_PARTITIONING_COUNT *
		  TUPLES_PER_CACHELINE) * sizeof(hpcjoin::data::CompressedTuple);
  ...
  innerPartitions = (hpcjoin::data::CompressedTuple *) hpcjoin::memory::Pool::getMemory(
	  innerOutputSize);
  ...
  uint64_t outerOutputSize = (outerPartitionSize +
	  hpcjoin::core::Configuration::LOCAL_PARTITIONING_COUNT *
		  TUPLES_PER_CACHELINE) * sizeof(hpcjoin::data::CompressedTuple);
  ...
  outerPartitions = (hpcjoin::data::CompressedTuple *) hpcjoin::memory::Pool::getMemory(
	  outerOutputSize);
  ...
  JOIN_DEBUG("Local Partitioning", "Partitioning inner partition of size %lu",
			 innerPartitionSize);
  partitionData(innerPartition, innerPartitionSize, innerPartitions, innerOffsets,
				innerHistogram);

  JOIN_DEBUG("Local Partitioning", "Partitioning outer partition of size %lu",
			 outerPartitionSize);
  partitionData(outerPartition, outerPartitionSize, outerPartitions, outerOffsets,
				outerHistogram);
  ...
  for (uint32_t p = 0; p < hpcjoin::core::Configuration::LOCAL_PARTITIONING_COUNT; ++p) {
	if (innerHistogram[p] > 0 && outerHistogram[p] > 0) {
	  hpcjoin::operators::HashJoin::TASK_QUEUE.push(
		  new hpcjoin::tasks::BuildProbe(innerHistogram[p],
										 innerPartitions + innerOffsets[p],
										 outerHistogram[p],
										 outerPartitions + outerOffsets[p]));
	}
  }
...
uint64_t *LocalPartitioning::computeHistogram(hpcjoin::data::CompressedTuple *tuples, uint64_t size) {
  ...
  for (uint64_t t = 0; t < size; ++t) {
	uint64_t idx = HASH_BIT_MODULO(tuples[t].value, MASK,
								   hpcjoin::core::Configuration::NETWORK_PARTITIONING_FANOUT +
									   hpcjoin::core::Configuration::PAYLOAD_BITS);
	++(histogram[idx]);
  }
...
uint64_t *LocalPartitioning::computePrefixSum(uint64_t *histogram) {
  ...
  uint64_t *prefix = (uint64_t *) calloc(hpcjoin::core::Configuration::LOCAL_PARTITIONING_COUNT,
										 sizeof(uint64_t));
...
void LocalPartitioning::partitionData(hpcjoin::data::CompressedTuple *input, uint64_t inputSize,
									  hpcjoin::data::CompressedTuple *output,
									  uint64_t *partitionOffsets, uint64_t *histogram) {

  cacheline_t inCacheBuffer[hpcjoin::core::Configuration::LOCAL_PARTITIONING_COUNT]
...
void LocalPartitioning::streamWrite(void *to, void *from) {
  ...
  JOIN_ASSERT(((uint64_t) to) % LOCAL_PARTITIONING_CACHELINE_SIZE == 0, "Local Partitioning",
			  "Stream destination not aligned");
  JOIN_ASSERT(((uint64_t) from) % LOCAL_PARTITIONING_CACHELINE_SIZE == 0, "Local Partitioning",
			  "Stream source not aligned");
